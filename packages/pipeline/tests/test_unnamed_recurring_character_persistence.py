from echotales.core.enums import AliasType, BlockType, ReferenceMode, SpanType
from echotales.core.models import Block, Chapter, Mention
from echotales.core.store import Store
from echotales.pipeline.resolve import resolve_novel


def _chapter(number: float, text: str) -> Chapter:
    return Chapter(
        novel_id="t",
        number=number,
        title=f"Chapter {int(number)}",
        source_href=f"{int(number)}.html",
        blocks=[Block(index=0, block_type=BlockType.PROSE, text=text)],
    )


def _mention(mid: str, text: str, *, chapter: float, group: str) -> Mention:
    return Mention(
        id=mid,
        novel_id="t",
        segment_id="s",
        chapter=chapter,
        offset=0,
        text=text,
        alias_type=AliasType.EPITHET,
        span_type=SpanType.NARRATION_ACTION,
        reference_mode=ReferenceMode.PRESENT,
        block_index=0,
        local_group_id=group,
    )


def test_recurring_unnamed_character_persists_across_chapters():
    store = Store(":memory:")
    store.add_novel("t", "T", "x.epub", "generic")
    store.add_chapter(_chapter(1.0, "The Guard Captain entered the hall."))
    store.add_chapter(_chapter(2.0, "The Guard Captain gave orders to the squad."))

    store.add_mentions(
        [
            _mention("m1", "Guard Captain", chapter=1.0, group="g1"),
            _mention("m2", "Guard Captain", chapter=2.0, group="g2"),
        ]
    )
    store.conn.commit()

    resolve_novel("t", store)

    m1 = next(m for m in store.get_mentions("t", 1.0) if m.text == "Guard Captain")
    m2 = next(m for m in store.get_mentions("t", 2.0) if m.text == "Guard Captain")

    assert m1.target_id is not None
    assert m2.target_id == m1.target_id
